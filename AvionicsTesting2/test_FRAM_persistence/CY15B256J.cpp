...
constexpr unsigned long wait_for_i2c_device{20};
...
bool CY15B256J::begin(uint8_t addr, TwoWire *theWire)
{
  ...
  if (manufID != 0x004)
  {
    Serial.println("Unexpected Manufacturer ID: " + String(manufID, HEX));
    return false;
  }
  if (prodID != 0x221)
  {
    Serial.println("Unexpected Product ID: " + String(prodID, HEX));
    return false;
  }
